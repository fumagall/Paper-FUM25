...
def Model(s, r, b, seed = None, n_buffer = 10_000):
    model = Lorenz63(s,
                r,
                b,
                seed)
...
from lib.correlation_integral import LongSpatialCI, TransformedCorrelationIntegral
...
class RefSpatialCI(TransformedCorrelationIntegral):
    ...
    @staticmethod
    def transform(timeseries: np.ndarray) -> np.ndarray:
        return timeseries
    
    @staticmethod
    def random_subset(rng, timeseries: np.ndarray) -> np.ndarray:
...
def worker(args):
    ...
    ci = RefSpatialCI("data/heikki_ref/Lorenz63_RefSpatialCI_ndata_600_nmul_1_seed_0.npz")
    rng = default_rng(seed)

    theta = np.array((10, 28, 8/3))
    timeseries = Model(*theta, rng.integers(0, 2**31)).get_timeseries(20_000) # type: ignore
    ...
    for t in samples:
        ...
        proposal_theta_timeseries = Model(*proposal_theta, rng.integers(0, 2**31)).get_timeseries(20_000)[0]
...
np.savez(f"results/mcmc/heikki_ref_rep_{sig:.4f}.npz", samples = np.array(mcmc_samples), acceptance_rate=np.array(acceptance_rates))