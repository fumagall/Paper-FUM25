...
from lib.correlation_integral import LongSpatial02CI, LongSpatialCI, TransformedCorrelationIntegral
...
n= 300 # how many timeseries to generate
mul = 100 # how much longer the long timeseries is compared to the short timeseries
seed = 0 
...
fname = f"data/heikki_long{"_dt_02" if args.use_02_model else "" }/{Model().ode.__class__.__name__}_{CI.__name__}_ndata_{n}_nmul_{mul}_seed_{seed}" 
...
def correlation_sum_worker(arg):
    ...
    return TransformedCorrelationIntegral.distance_minmax(CI.random_subset(rng, timeseries_shm[i]), CI.random_subset(rng, long[j])) # type: ignore
...
def correlation_sum_worker2(arg):
    ...
    return TransformedCorrelationIntegral.correlation_sum_heikki(CI.random_subset(rng, timeseries_shm[i]), CI.random_subset(rng, long[j]), bins) # type: ignore
...
warn("only saves single timeseries")
np.savez(fname, timeseries = long_timeseries[:1], mean = mean_subset, cov = cov_subset, rs = bins) 
