...
parser.add_argument("--n_samples", type=int, help="Number of different reservoir", default=1000)
...
with Pool(48) as p:
    ...
    n_radii = 100
    spectral_radii=np.linspace(0, 1.5, n_radii)
    collected = []

    inputs = list(zip(*zip(*product(spectral_radii, [i_model]*n)), ss.spawn(n_radii * n)))

    collected += list(zip(*tqdm(p.imap(worker, inputs), total=n*n_radii, smoothing=0))),
    ...
    np.savez(f"results/spectral_radius_n_nodes_{args.n_nodes}/{name}{"_dt_02" if args.use_02_model else "" }_{Model(0,0).ode.__class__.__name__}.npz", measures = measures, names = names)
