...
# models += lambda seed = None, n_buffer = 10000: JAU24a(seed, n_buffer),
...
def worker(inputs):
    ...
    ci = CI(f"data/heikki_long{"_dt_02" if args.use_02_model else "" }{sub}/Lorenz63_{CI.__name__}_ndata_300_nmul_100_seed_0.npz")
...
np.savez(f"results/mcmc/heikki_long{"_dt_02" if args.use_02_model else "" }{sub}_{sig:.4f}.npz", samples = np.array(mcmc_samples), acceptance_rate=np.array(acceptance_rates))